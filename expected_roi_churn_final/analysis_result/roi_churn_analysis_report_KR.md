...
- **Mann-Whitney U 검정**:
  ...
  - t-test보다 Mann-Whitney 검정이 더 유의미한 결과를 보이는 것은 데이터가 정규분포를 따르지 않기 때문

- **로지스틱 회귀 분석**:
  - 기대 수익률 계수: -0.583 (p < 0.001)
  - 오즈비(Odds Ratio): 0.558
  - 이는 기대 수익률이 1단위 증가할 때마다 이탈 확률이 약 44.2% 감소함을 의미
  - 모델 성능: 
    - AUC: 0.614 (예측력이 무작위 추측보다는 낫지만 강력하지 않음)
    - 정확도: 60.3%
    - 정밀도: 61.8%, 재현율: 59.7%

**히트맵 해석**:
- 히트맵은 기대 수익률과 이탈 상태(IS_CHURNED) 간의 명확한 상관관계(-0.308)를 시각적으로 보여줌
- 푸른색 음영은 음의 상관관계를 나타내며, 이는 기대 수익률이 높을수록 이탈 가능성이 낮아짐을 의미
- 대각선 요소(1.000)는 각 변수가 자기 자신과 완벽한 상관관계가 있음을 표시
- 이 히트맵은 이전 분석에서 제시한 약한 상관관계보다 더 강한 관계(-0.308)를 보여주며, 이는 이상치를 제거한 데이터셋을 사용했기 때문

*참고: 로지스틱 회귀 분석의 시각화 이미지는 현재 누락되어 있습니다. 향후 분석에서 기대 수익률에 따른 이탈 확률 곡선 등을 포함한 시각화 자료를 추가할 예정입니다.*
...
- 기대 수익률을 기반으로 이탈 확률을 모델링하기 위한 로지스틱 회귀
...
- **통계적 유의성**: Mann-Whitney U 검정에서 두 그룹 간 기대 수익률 분포 차이가 통계적으로 유의미함
...
최종적으로, 예측 수익률은 이탈과 중간 수준의 상관관계를 가지며, 이는 이탈 예측에 유용한 지표로 활용될 수 있습니다. 그러나 최적의 예측을 위해서는 거래 활동과 같은 다른 요소들과 함께 고려하는 것이 중요합니다. 데이터 분석 시 이상치 처리의 중요성이 확인되었으며, 이상치 제거 후 기대 수익률과 이탈 간의 관계가 더욱 명확해졌습니다. 또한, 기대 수익률과 이탈 간의 관계는 단순한 선형 관계가 아닌 복잡한 패턴을 보일 수 있으므로, 다양한 수익 구간에 맞춤화된 리텐션 전략이 필요합니다.
